...
#### Authors : [Cristopher Benge](https://cbenge509.github.io/) | [William Casey King, PhD](https://jackson.yale.edu/person/casey-king/) 
...
#### Highlight of key files included in this repository:
  ...
  |[Presentation](/presentation/King_Benge_W207_Final.pdf) | Presentation of high-level approach and results|
...
The TRAIN dataset provided by the competition organizers appears to derive from two distinct sources: one where all 15 landmark labels are present, and another where only 4 are present (note: there are a few images thorughout that have some other count of missing label information, but they are the exception).  Initially, we were able to achieve a single best-model RMSE score of **1.43812** by addressing all missing labels through interpoloation of their `np.nanmean()` keypoint average location.
